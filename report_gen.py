...
def to_hex(rgb: tuple) -> str:
    """Converts RGB tuple to Hex string for Matplotlib compatibility."""
...
class StrategicReport(FPDF):
    ...
    def header(self):
        """Renders the standard enterprise header."""
    ...
    def footer(self):
        """Renders the standard enterprise footer."""
        self.set_y(-12)
        self.set_font('helvetica', 'I', 8)
        self.set_text_color(150, 150, 150)
        self.cell(0, 10, f'Page {self.page_no()}', 0, 0, 'C')

    def safe_text(self, text: Any) -> str:
        """Sanitizes text to prevent Latin-1 encoding errors in FPDF."""
    ...
    def draw_sidebar(self, metrics: Dict[str, Any]):
        """Renders the telemetry sidebar on the right margin."""
        x_base = 140
        y_base = 20
        w = 70
        h = 277
        ...
        def sb_row(label: str, val: Any, bold: bool = False, col: tuple = C_TEXT):
            nonlocal curr_y
            self.set_xy(pad, curr_y)
            self.set_font('helvetica', '', 9)
    ...
    def render_table(self, table_lines: List[str]):
        """Parses and renders Markdown tables into FPDF standard cells."""
        data = []
    ...
    def write_smart_content(self, text: str):
        """Parses Markdown text and applies intelligent PDF typography styling."""
...
def generate_pdf(metrics: Dict[str, Any], ai_summary: str) -> bytes:
    """Generates the comprehensive PDF payload including Matplotlib visualizations."""
    pdf = StrategicReport()
    pdf.add_page()
